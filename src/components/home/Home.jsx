...
export const Home = () => {
  ...
  useEffect(() => {
    const covidList = async () => {
      setIsLoading(true);
      await axios.get(`http://localhost:8000/covid/covid-api`)
      .then((res) => {
        const data = res.data;
        setIsLoading(false);
  ...
  return (
    <div className="w-full h-screen bg-grey-50 dark:bg-gray-800 transition duration-300">
      ...
      <div className="mx-10 py-20 flex justify-between">
        {isLoading && (
          <div role="status" className="fixed top-50 w-full h-full">
        ...
        <div className="w-full h-full mt-20 relative overflow-x-auto shadow-md sm:rounded-lg">
          {!isLoading && (
            <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400 transition duration-300">
              ...
              <tbody>
                {isCovidData &&
                  isCovidData?.map((item, index) => {
                    return (
                      <tr
                        className="bg-white border-b dark:bg-gray-900 dark:border-gray-700 transition duration-300"
                        key={index}
                      >
                        <th
                          scope="row"
                        ...
                        <td className="px-6 py-4">{item?.recovered}</td>
                        <td className="px-6 py-4">{item?.deaths}</td>
